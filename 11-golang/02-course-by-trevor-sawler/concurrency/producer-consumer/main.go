...
// Producer is a type for structs that holds two channels: one for pizzas,
// with all information for a given pizza order including whether it was made
// successfully, Another to handle end of processing (when we quit the channel)
...
// Close is simply a method of closing the channel when we are done
// with it. For example If something is pushed to the quit channel
...
// makePizza attemps to make a pizza. We generate a random number from 1-12,
...
// cals makePizza to try to make one order each time it iterates through
// the for loop. It executes until it receives something on the quit channel.
// The quit channel does not reecive anything until the consumer
// sends it (when the number of orders is greater than or equest
// to the constant NumberOfPizzas).
